...
class TestEnsureKst:
    def test_naive_gets_kst(self):
        naive = datetime(2026, 9, 9, 15, 15)  # noqa: DTZ001 — naive 입력이 테스트 대상
    ...
    def test_utc_aware_not_silently_shifted(self):
        ...
        utc_dt = datetime(2026, 9, 9, 6, 0, tzinfo=UTC)
        assert ensure_kst(utc_dt).utcoffset() == UTC.utcoffset(None)


class TestKstDate:
    def test_utc_evening_is_next_kst_day(self):
        # KST 새벽 경계 — UTC 17:00 == KST 다음날 02:00
        assert to_kst_date(datetime(2026, 8, 10, 17, 0, tzinfo=UTC)) == date(
            2026, 8, 11
        )

    def test_utc_morning_same_kst_day(self):
        assert to_kst_date(datetime(2026, 8, 10, 3, 0, tzinfo=UTC)) == date(2026, 8, 10)

    def test_naive_treated_as_kst(self):
        # naive 는 KST 로 해석 — 23:30 은 그대로 당일
        naive = datetime(2026, 8, 10, 23, 30)  # noqa: DTZ001 — naive 입력이 테스트 대상
        assert to_kst_date(naive) == date(2026, 8, 10)
