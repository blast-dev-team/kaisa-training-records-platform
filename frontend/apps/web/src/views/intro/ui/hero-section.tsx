...
function StepRow({ order, label, active }: { order: number; label: string; active: boolean }) {
  return (
    <div className="flex w-full items-center gap-4 mobile:gap-3">
  ...
  return (
    <div
      className={cn(
        ...
        tone === "info" ? "border-primary-400 bg-primary-50" : "border-red-500 bg-red-50",
      )}
    >
      <p
        className={cn(
          ...
          <div className="flex w-full flex-col gap-6 rounded-[20px] border border-solid border-gray-200 bg-white p-8 mobile:p-6">
            <div className="flex w-full flex-col gap-2">
              ...
              <div className="flex w-full flex-wrap items-center gap-2 mobile:justify-between">
                ...
                <Checkbox
                  size="s"
                  className="hidden mobile:inline-flex mobile:items-center"
                  checked={isAgreed}
                  onChange={(event) => setIsAgreed(event.target.checked)}
                >
                  개인정보 수집·이용에 동의합니다.
            ...
            <div className="flex w-full items-start gap-6 mobile:flex-col mobile:gap-8">
              ...
              <div className="flex min-w-px flex-1 flex-col gap-3 mobile:w-full">
                ...
                <Button
                  size="m"
                  fullWidth
                  className="hidden mobile:flex mobile:flex-1"
                  disabled={!isAgreed}
                  onClick={() => setIsVerificationOpen(true)}
                >
                  교육내역확인서 발급
                ...
                <div className="flex w-full flex-col gap-3">
                  {ISSUE_STEPS.map((label, index) => (
                    <StepRow key={label} order={index + 1} label={label} active={index === 0} />
                  ))}
                </div>
              </div>
              ...
              <div className="flex min-w-px flex-1 self-stretch flex-col gap-3">
                <div className="flex w-full flex-col gap-3">
                  ...
                  <Button
                    ...
                    size="l"
                    fullWidth
                    className="border-gray-800 text-gray-800 mobile:hidden"
                    onClick={() => navigate("/verification-no-auth")}
                  >
                    확인서 진위확인
                  </Button>
                  {/* 모바일 — gray-800 filled (node 125:8510) */}
                  <Button
                    variant="filled"
                    color="black"
                    size="m"
