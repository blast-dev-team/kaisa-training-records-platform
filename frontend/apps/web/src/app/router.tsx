  ...
  // 인증 완료 후 — 좌측 사이드바 (교육이력 조회 · 발급 결제 내역 · 진위확인)
  {
    ...
    children: [
      ...
      },
      {
        ...
        lazy: async () => {
          const { PaymentHistoryPage } = await import(
            "@/src/views/payment-history"
          );
    ...
    children: [
      {
        index: true,
        lazy: async () => {
          const { TrainingHistoryDetailPage } = await import(
            "@/src/views/training-history-detail"
          );
      ...
      },
      {
        path: "complete",
        lazy: async () => {
          const { IssuanceCompletePage } = await import(
            "@/src/views/issuance-complete"
          );
